...
namespace Plesanter.CLB.Designer.Lib.AI
{
    public class ModuleCreator : IAIChat
    {
        ...
        public string Explanation => "モジュールを作成するためのチャットです";
...
## DBのテーブルとマッピングしない表示専用のModuleも作れます。チャート表示やダッシュボード等、データを参照表示するだけの場合です。この場合DbTableとDataSourceNameは空文字にし、IdフィールドやSubmitボタンは不要です。またListLayoutDesignのElementsにフィールドを入れないでください。フィールドを入れると一覧表示モジュールと認識されてしまいます。
...
- DateTime
- Time
- ApexChart（棒グラフ/折れ線グラフ/面グラフ/散布図/ヒートマップ）
- ApexRadialChart（円グラフ/ドーナツチャート/極座標チャート）
...
### ApexChartはデータを検索してチャート表示します。SearchConditionで対象Moduleを指定し、CategoryFieldでX軸、SeriesでY軸のNumberFieldを指定します。DBカラムは持ちません。
### ApexRadialChartは円形チャート（円グラフ、ドーナツ等）を表示します。SearchConditionで対象Moduleを指定し、CategoryFieldでカテゴリ、SeriesFieldで値のNumberFieldを指定します。DBカラムは持ちません。
...
"));
            ...
            bool IsUseField(Type type) => new List<Type> {
                typeof(IdFieldDesign), typeof(BooleanFieldDesign), typeof(TextFieldDesign), typeof(NumberFieldDesign), typeof(SelectFieldDesign), typeof(LinkFieldDesign),
                typeof(ListFieldDesign), typeof(FileFieldDesign), typeof(DateFieldDesign), typeof(DateTimeFieldDesign), typeof(TimeFieldDesign),
                typeof(ApexChartFieldDesignBase)
...
- Module
{
  ...
  ""DbTable"": テーブル名（DBと結びつかない表示専用モジュールの場合は空文字）,
  ""CanCreate"": true,
  ""CanUpdate"": true,
  ""CanDelete"": true,
  ""Fields"": []
}
DBと結びつかない表示専用モジュール（チャート表示のみ等）の場合：
- DbTableは空文字""""にする
- DataSourceNameも空文字""""にする
- IdフィールドやSubmitボタンは含めない
- ListLayoutDesignのElementsにフィールドを入れないこと。ListLayoutDesignにフィールドが入っていると一覧表示モジュールと認識されてしまう
...
- ApexChart
データを検索してチャート（棒グラフ/折れ線グラフ/面グラフ/散布図/ヒートマップ）として表示する読み取り専用フィールドです。DBカラムは持ちません。
SeriesTypeはデフォルトのチャートタイプです。Series内の各系列で個別にTypeを指定できます。
CategoryFieldはX軸に使うフィールド名、SeriesのNameはY軸に使うNumberFieldのフィールド名です。
{
  ""Name"": Field名,
  ""SeriesType"": ""Bar"",
  ""Series"": {
  ...
  ""DisplayName"": チャートのタイトル,
  ""CategoryField"": X軸に使うフィールド名,
  ""CategoryFormat"": null,
  ""SeriesFractionDigits"": 0,
  ""ShowLegend"": true,
  ""TypeFullName"": ""Codeer.LowCode.Bindings.ApexCharts.Designs.ApexChartFieldDesign""
}
SeriesTypeとSeries内のTypeに使える値: Bar, Line, Area, Scatter, Heatmap

- ApexRadialChart
データを検索して円形チャート（ドーナツ/円グラフ/極座標）として表示する読み取り専用フィールドです。DBカラムは持ちません。
SeriesFieldに1つのNumberFieldのフィールド名を指定します。
{
  ""Name"": Field名,
  ""SeriesType"": ""Donut"",
  ""SeriesField"": NumberFieldのフィールド名,
  ""SearchCondition"": {
    ""ModuleName"": Module名,
    ""Condition"": {
      ""IsOrMatch"": false,
